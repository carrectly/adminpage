  ...
  if (value) {
    return (
      <div>
        ...
        <br />
        <span className="subtext">{moment(value).format('hh:mm A')}</span>
      </div>
    );
  } else {
    return <div />;
...
export const OrderDetailsCell = ({ value }) => <Link to={`/singleorder/${value}`}>Details</Link>;

export const DeleteOrderCell = ({ value }) => {
  const dispatch = useDispatch();
  const handleClick = (evt) => {
    evt.preventDefault();
    if (window.confirm('Are you sure you want to delete this order from the database?')) {
      if (evt.target.id) {
...
export const LocationCell = ({ value }) => {
  return (
    <a onClick={() => window.open(`https://maps.google.com/?q=${value}`, '_blank')}>
      <EnvironmentFilled />
      {value}
    </a>
  );
...
export const DeleteCustomerCell = ({ value }) => {
  ...
  const handleClick = (evt) => {
    evt.preventDefault();
    if (window.confirm('Are you sure you want to delete this customer from the database?')) {
      if (evt.target.id) {
...
export const ConciergeCell = ({ value, dropDown = false }) => {
  ...
  if (dropDown) {
    return (
      <Tag color={value.tagColor} key={value.id} style={{ margin: '0px', border: '0px' }}>
        {value.name ? value.name : value.firstName} <DownOutlined />
      </Tag>
    );
  }
  ...
  return (
    <Tag color={driverObj.tagColor} key={driverObj.name}>
      {driverObj.name ? driverObj.name : driverObj.firstName}
    </Tag>
  );
};
...
export const GoogleVoiceLinkCell = ({ value }) => (
  <Popover placement="bottom" content="Click to view google voice conversation" trigger="hover">
    <a
      onClick={() =>
        window.open(`https://voice.google.com/u/1/messages?itemId=t.%2B${value}`, '_blank')
      }
    >
      {value}
      <GooglePlusCircleFilled />
